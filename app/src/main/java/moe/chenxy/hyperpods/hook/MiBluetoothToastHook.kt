...
object MiBluetoothToastHook : YukiBaseHooker(){
    private const val ISLAND_LEFT_POD_ICON = "miui.focus.pic_left_pod"
    private const val ISLAND_RIGHT_POD_ICON = "miui.focus.pic_right_pod"
    @Volatile
    ...
    override fun onHook() {
        ...
        fun buildIslandTicker(batteryParams: BatteryParams): String {
            return listOfNotNull(
                batteryParams.left?.takeIf { it.isConnected }?.battery?.let { "$it%" },
                batteryParams.right?.takeIf { it.isConnected }?.battery?.let { "$it%" }
            ).joinToString("  ")
        }

        fun buildIslandData(batteryParams: BatteryParams): JSONObject {
            val leftInfo = batteryParams.left?.takeIf { it.isConnected }?.let {
                islandApi.imageTextInfo(
                    picInfo = islandApi.picInfo(pic = ISLAND_LEFT_POD_ICON),
                    textInfo = islandApi.TextInfo(title = "${it.battery}%"),
                    type = 1
                )
            }
            val rightInfo = batteryParams.right?.takeIf { it.isConnected }?.let {
                islandApi.imageTextInfo(
                    picInfo = islandApi.picInfo(pic = ISLAND_RIGHT_POD_ICON),
                    textInfo = islandApi.TextInfo(title = "${it.battery}%"),
                    type = 1
                )
            }
